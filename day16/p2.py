...
def main():
    with open('day16.txt') as data:
        ...
        @functools.cache
        def find_best(opened, cycles, pos, elephant):
            best = 0
            for valve in opened:
                ...
                if dist < cycles:
                    ...
                    op = set(opened)
                    op.remove(valve)
                    x += find_best(frozenset(op), next_cycles, valve, elephant)
                if x > best:
                    best = x
            if elephant:
                x = find_best(opened, 26, 'AA', False)
                if x > best:
                    best = x

            return best

        print(find_best(frozenset(flows), 26, 'AA', True))
